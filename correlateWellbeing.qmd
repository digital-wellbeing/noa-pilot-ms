---
title: "Well-being is linked to the perceived value of video games, but not playtime itself: An analysis of Nintendo digital trace data in a casual adult audience"
shorttitle: "Platform-level video gaming and well-being"
author:
  - name: Nick Ballou
    corresponding: false
  ...
  - name: Andrew K Przybylski
    ...
    roles:
      ...
      - editing
abstract: "Studies on video games and well-being often rely on self-report measures or data from a single game. This study of US adults (n = 648) recruited via Prolific analyzes the impact of time spent on over 150 1st-party Nintendo Switch games (totaling 140k hours) on life satisfaction, affective valence, depressive symptoms, and general mental well-being. We replicate previous findings that playtime over the past 2 weeks does not predict well-being, and extend these null findings to a wider range of timescales (1 hour to 1 year). Results suggest that relationships, if present, dissipate within 2 hours of gameplay. Although playtime was not related to well-being, players' assessments of the value of game time---so called gaming life fit---is moderately linked. Our non-causal findings suggest substantial confounding would be needed to shift a meaningful effect to null. Results emphasize the importance of defining the gaming population of interest, collecting data from more than one game, and focusing on how players integrate gaming into their lives rather than the amount of time spent."
...
mytheme <- list(
  theme_bw() +
    theme(
      ...
      axis.text.y = element_text(size = 12, hjust = .5, vjust = -.015),
      axis.text.x = element_text(size = 12),
      axis.title.x = element_text(hjust = .45, size = 14, face = "bold"),
      axis.title.y = element_text(hjust = .45, size = 14, face = "bold"),
...
We were also interested in understanding how different timescales relating to play and well-being might be linked. To this end, our second hypothesis focused on how different time lags between when someone plays and is asked about well-being influence the strength of their association. While preregistered, the large number of models implicated in our analysis plan is prohibitively large to be aptly framed as a narrow hypothesis test. We therefore operationalize this as an exploratory research question: How does the relationship between playtime and well-being differ across different gaming observation windows ranging from 1 hour to 1 year?
...
ethnicities <- datDemo |> 
  filter(pid %in% dat$pid & ethnicity != "DATA_EXPIRED") |> 
  count(ethnicity, sort = TRUE) |>
  mutate(p = n / sum(n)) |>
  mutate(x = str_glue("{ethnicity} ({number(n, big.mark = '')}, {percent(p)})")) |>
  slice(1:4) |>
  pull(x) |>
  knitr::combine_words()

jobs <- datDemo |> 
  filter(pid %in% dat$pid & employmentStatusProlific != "DATA_EXPIRED") |> 
  count(employmentStatusProlific, sort = TRUE) |>
  mutate(p = n / sum(n)) |>
  mutate(x = str_glue("{employmentStatusProlific} ({number(n, big.mark = '')}, {percent(p)})")) |>
  slice(1:4) |>
  pull(x) |>
  knitr::combine_words()
...
dat |> 
  ...
  summarise(
    n = n(),
            `Median age (SD)` = paste0(median(age, na.rm = TRUE), " (", round(sd(age, na.rm = TRUE), 2), ")")
    ) |> 
  flextable(cwidth = c(2.5, 1, 1)) %>% 
  theme_apa() 
```

Demographic information about participants is shown in @tbl-demographics. All participants were residents of the US, and were majority white: `r ethnicities`. Approximately half were in full-time employment: `r jobs`.
...
```{r}
#| label: fig-playtime
#| fig-cap: "Density of playtime across each of the observation periods from 1 hour to 1 year."
#| fig-width: 8
#| fig-height: 6
#| fig-align: center
#| fig-pos: "t"
...
#   pivot_longer(cols = -pid, names_to = "timePeriod", values_to = "played") %>%
#   group_by(timePeriod) %>%
#   summarise(
#     count = sum(played),
#     prop = sum(played, na.rm = TRUE)/ n(),
#   ) |> 
#   mutate(
#     timePeriod = factor(timePeriod, levels = playedDict),
#     timePeriod = fct_rev(fct_recode(timePeriod, !!!playedDict)),
#   ) |> 
#   arrange(timePeriod) |> 
#   mutate(index = 1:n()) |> 
#   ggplot(aes(y = timePeriod, x = prop)) +
#   geom_bar(stat = "identity", fill = "#164187") + 
#   geom_text(aes(label = paste0(round(prop*100, 0), "% (", count, ")"),
#                 hjust = ifelse(index <= 4, 1.1, -.1),
#                 color = index <= 4), 
#             size = 4) +
#   mytheme + 
#   theme(plot.margin = margin(3,1.5,3,0, "cm"),
#         legend.position = "none") +
#   scale_x_continuous(breaks = seq(0, 1, .25), labels = scales::percent) +
#   scale_color_manual(values = c("black","white")) +
#   labs(fill = "", x = "Proportion of\nparticipants with\nany playtime", y = "")
# 
# playtimeHist <- dat |> 
#   filter(!is.na(recordedDate)) |> 
#   select(pid, recordedDate, matches("playtime\\d")) |> 
#   pivot_longer(cols = starts_with("playtime"), names_to = "timePeriod") |>
#   filter(
#     value > 0 & # we plot average playtime only for players with some logged data
#       value < 2 # for some reason, we need to set the xmax limit here, as filtering later causes issues with proportion
#     ) |> 
#   mutate(
#     timePeriod = factor(timePeriod, levels = playtimeDict),
#     timePeriod = fct_recode(timePeriod, !!!playtimeDict)
#   ) |> 
#   mutate(value = value*60) |> # convert to minutes for a more granular plot
#   ggplot(
#     aes(x = value, 
#         fill = timePeriod, 
#         group = timePeriod,
#         y = after_stat(density*width)) # width * density gives proper proportions in facet_wrapped plots
#   ) + 
#   geom_histogram(binwidth = .2*60, color = "black") +
#   scale_y_continuous(limits = c(0, .65), labels = scales::percent) +
#   scale_x_continuous(limits = c(-10, 120), breaks = seq(0, 120, 30)) +
#   scale_fill_viridis_d() +
#   mytheme + 
#   labs(
#     x = "Minutes of Daily Play\n(where playtime window < 24 hours,\nraw minutes)",
...
# 
# plot_grid(played, playtimeHist, ncol = 2, rel_widths = c(4, 6))
...
dat |> 
  ...
  mutate(
    playtimeCat = factor(case_when(
    value > 40/60 ~ ">40\nmin/day",
    value > 30/60 ~ "30-40\nmin/day",
    value > 20/60 ~ "20-30\nmin/day",
    value > 10/60 ~ "10-20\nmin/day",
    value > 0 ~ "0-10\nmin/day",
    value == 0 ~ "No play during\nobservation\nperiod"),
    levels = c("No play during\nobservation\nperiod", "0-10\nmin/day", "10-20\nmin/day", "20-30\nmin/day", 
               "30-40\nmin/day", ">40\nmin/day"))
  ) |> 
  group_by(playtimeCat, timePeriod) |> 
  summarise(count = n(),
            prop = n()/nrow(dat)) |> 
  ungroup() |> 
  complete(timePeriod, playtimeCat, fill = list(count = 0, prop = 0)) |> 
  ggplot(aes(playtimeCat, timePeriod, fill = count)) + 
  geom_tile() +
  geom_text(aes(label = count), color = "#404040") +
  labs(x = "", y = "Observation Window", fill = "Count") +
  scale_fill_gradient(low="white", high="blue") +
  theme_half_open()

```

Given the lack of holistic or platform-level data available in the literature, our first goal was to simply describe the volume of play. This is visualized in @fig-playtime, which show that despite a total play volume of more than 140k hours, our sample was largely minimally engaged with 1st party Nintendo games. During the 2 weeks prior to survey completion, 40% of the sample had no sessions logged at all. The top 10% of players were highly engaged, playing more than 60 minutes per day on average. Sessions of a game lasted on average `r round(median(datRaw$duration, na.rm = T), 1)` minutes \[10th percentile: `r quantile(datRaw$duration, .1)`; 90th percentile: `r quantile(datRaw$duration, .9)`\].
...
dat |> 
  ...
  geom_point(size = .6) +
  geom_smooth(method='lm', formula = y~x, level = .99, color = "darkred") +
  scale_x_sqrt(breaks = c(0, 15, 60, 120, 240, 420)) +
  labs(x = "Minutes of play per day\nduring previous 2 weeks (Square Root Axis)", y = "Well-being (1-5 scale)") +
  mytheme + 
...
fittedModels |> 
  ...
  labs(
    ...
    y = "Association between\noutcome and playtime\nin observation period"
  ) +
  theme_linedraw() +
...
The idea that time is the metric by which we evaluate the associations linking video game play to well-being will be with us for some time. Our study directly tested this idea and although we were not able to establish a causal link, it challenges the notion that increased playtime significantly affects well-being, for better or for worst. We report correlations that are in most cases too small to practically matter, and demonstrate that significant confounding would be required to suppress a true causal effect to produce the null associations observed here. This is improbable but not impossible, and we believe our results lend weight to calls for scholars and health practitioners to embrace the gradual shift of focus towards quality, rather than quantity of video game play as the key factor for player health. If this can be done while simultaneously improving data quality and access, a coherent and evidence-based method for studying the complex relationships linking video game play and well-being will be possible.
