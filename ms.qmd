...
dat |> 
  select(playtime1d, playtime2wk, playtime3mo, playtime1yr) |> 
  # convert back to total playtime in hours, rather than minutes per day
  # mutate(
  #   playtime1d = playtime1d*24,
  #   playtime2wk = playtime2wk*24*14,
  #   playtime3mo = playtime3mo*24*91,
  #   playtime1yr = playtime1yr*24*36
  # ) |> 
  # scale_x_continuous(labels = scales::cut_si()) +
  pivot_longer(cols = everything(), 
               names_to = "timewindow", 
               names_pattern = "playtime(.*)", 
               values_to = "playtime") |> 
  mutate(timewindow = fct_recode(timewindow,
                                 "1 Day" = "1d",
                                 "2 Weeks" = "2wk",
                                 "3 Months" = "3mo",
                                 "1 Year" = "1yr"),
         timewindow = fct_relevel(timewindow, 
                                  "1 Day", 
                                  "2 Weeks", 
                                  "3 Months", 
                                  "1 Year"),
         # playtime = if_else(playtime == 0, map_dbl(playtime, ~ sample(c(0, -.1, -.2, -.3), 1)), playtime)
  ) |> 
  ggplot(aes(x = timewindow, y = playtime, fill = timewindow)) +
  stat_slab(
  ...
  labs(x = "Observation Period", y = "Daily Hours of Playtime") +
  scale_y_continuous(
    trans = "sqrt",
    breaks = c(0, .2, .5, 1, 2, 3, 4, 5),
    limits = c(0, 5)
    # limits = c(-.3, 5),
  ) +
  scale_x_discrete(expand = c(0, 0)) +
  coord_cartesian(xlim = c(.3, 4.3)) +
  mytheme +
  theme(legend.position = "none") +
  scale_fill_viridis_d()
